...
**让网页版 ChatGPT、Claude、Grok 直接读写你电脑上、或你 VPS 上的项目目录。改文件、跑命令之前,先在本机经你确认。**
...
- 项目在 VPS 上也一样。把那台机器接进来,AI 直接在服务器上改代码、跑测试,批准还是
  在你这台电脑上点。见[远程机器](#远程机器)。
- 明天新开一个对话,AI 接着上次的做:做到哪了、定了什么、下一步是什么。Fylane 按目录把这些
  记在你电脑上,交给下一个对话。
...
VPS 那边的 Fylane 不对外开任何端口,只有你这台电脑通过 ssh 能找到它。AI 平台那边不用改
任何配置,连的还是同一个地址。
...
3. **决定什么必须问你。** 默认是「每个目录问一次」:第一条普通命令要你点头,之后同一个目录
   ...
   命令都不问。文件写入有自己的一档梯子:每次都问、新建文件直接过、或者写入不再询问;不管
   选哪档,删除和敏感路径照样停下,每次写入都留撤销副本。
...
新开一个对话不用从头讲。Fylane 给每个目录记两样东西:一页当前状态(目标、进展、下一步、
已定的决定、待定的问题),和 AI 边做边写的一条笔记流水。

- **读不用你管。** 新对话里 AI 做的第一件事是向 Fylane 问这个目录,回答里会告诉它有记忆
  可读。说一句「接着上次做」通常就够了。就算一条笔记都没有,它也能看到上次写了什么、跑了什么。
- **写靠 AI 自觉。** 它被告知做完一件事、定了一件事就记一条,但没有东西强制它。一段工作
  收尾时说一句「记一下」。
- **新对话没接上,** 就说「先看看这个目录的记忆」。
- **不会越攒越大。** 状态页每次整页改写、从不追加,第一周和第一百周新对话读到的都是那几 KB。
  旧笔记可以折成一条摘要,原文仍能搜到。

「记忆」页能看到这两样。你可以直接改状态页、删掉某条笔记、整体导出成 Markdown,或者清空。
记忆存在目录所在那台机器的 Fylane 数据里,不写进你的项目文件。
...
| 记 | `memory_recall` `memory_note` `memory_search` `memory_read` `memory_compact`,每个目录一页当前状态加一条笔记流水,下一个对话从这里开始 |
...
| | |
| --- | --- |
| 我的代码会传到服务器上吗? | AI 读到的内容会到 AI 平台那边,和你手动贴过去一样。除此之外哪儿都不去,中间的隧道或中继什么都不存。 |
| AI 会不会把我的项目删了? | 删除进本机的回收区。目录本身和 `.git` 永远删不掉。删一个里面有文件的目录要确认两次。 |
| 我不在电脑旁边怎么办? | 请求在窗口里等着,AI 会被告知「等待批准」。你不点,什么都不会发生。嫌慢可以选「每个目录一次」:平常的活不再问,危险命令照样拦。 |
| 它什么命令都能跑吗? | 只接受程序加参数,不接受 shell 命令行。`rm -rf`、`sudo`、杀进程、写系统目录在任何档位都拒绝。macOS 和 Linux 上由操作系统把进程关在这个目录里。 |
| AI 换个对话还记得这个项目吗? | 记得。每个目录有一页当前状态(目标、进展、下一步、决定)和 AI 边做边写的笔记,新对话从这里开始。这些存在 Fylane 自己的数据里,不写进你的项目文件,你随时能看能删。 |
| 项目在 VPS 上能用吗? | 能。Fylane 走你现有的 ssh 连过去,在那台机器上跑一份自己,批准还是在这台电脑上点。 |
| 支持哪些 AI? | ChatGPT、Claude、Grok 上面三步就能接上。任何走 HTTP 的 MCP 客户端都能用同样的方式连。 |
